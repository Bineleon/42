...
/*   Updated: 2024/07/17 01:49:12 by neleon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_tile(t_data *game, char tile, int x, int y)
{
	...
	else if (tile == EXIT)
	{
		if (game->map->collec == game->collected)
		{
			...
			img_to_win(game, game->textures[5], x * IMG_SIZE, (y + 1) * IMG_SIZE);
		}
		else
			img_to_win(game, game->textures[0], x * IMG_SIZE, (y + 1) * IMG_SIZE);
	}
...
void	display_map(t_data *game)
{
	...
	while (y < game->map->line_count)
	{
		...
		while (x < game->map->col_count)
		{
			...
			{
				img_to_win(game, game->textures[4], x * IMG_SIZE, (y + 1) * IMG_SIZE);
